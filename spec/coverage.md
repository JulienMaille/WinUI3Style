...
| `QMenu` / `QMenuBar` | MenuFlyout / menu bar extension | Direct + extension | partial — reopened (#5, #6) | check, icon, shortcut, submenu and long-text contracts exist; open-menu density invalidation needs reproduction (#5). Opening motion currently uses a vertical 12 px slide with OutCubic, including lateral submenus; anchor-side direction, pinned easing, interruption and live start/mid/end evidence remain unresolved (#6). Acrylic requires a live compositor (opaque fallback offscreen); the prior AnimateWindow measurement claim lacks a reproducible evidence record. |
