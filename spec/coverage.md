...
| `QMenu` / `QMenuBar` | MenuFlyout / menu bar extension | Direct + extension | source-audited | check, icon, shortcut, submenu, long text, hover/down, acrylic popup surface on a live compositor (opaque fallback offscreen), open slide+fade 167 ms from the anchor side (AnimateWindow is a no-op on visible HWNDs, measured) |
